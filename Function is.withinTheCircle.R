#本函数用于判断一个点（vector）或者一系列的点（data.frame）是否在一个指定的圆形里面
#圆形由圆心center（vector）和半径radius（实数）定义
is.withinTheCircle = function(center,radius,p,tie = FALSE){
  if(tie){return(((center[1]-p[1])**2+(center[2]-p[2])**2) <= radius**2)}else{
    return(((center[1]-p[1])**2+(center[2]-p[2])**2) < radius**2)
  }
print('yes with you')}
